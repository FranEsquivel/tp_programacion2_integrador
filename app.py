...
def get_connection():
    ...
    return psycopg2.connect(
        ...
        dbname=os.getenv("DB_NAME", "flask_crud1"),
...
def index():
    ...
    order_by = columnas_validas.get(orden, "id ASC")

    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(f"SELECT * FROM estudiantes ORDER BY {order_by}")
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                WHERE dni ILIKE %s OR apellido ILIKE %s OR nombre ILIKE %s OR carrera ILIKE %s
                ORDER BY id ASC
                """,
